...
import { Cpu, Rocket, GraduationCap, Bird, Sparkles, Users, BookOpen, Lightbulb, Share2, Lock, Brain, Wrench } from "lucide-react";
...
const curriculumPrograms = [
  ...
  },
  {
    ...
    description: <>
      ...
      <ul>
        {[
          ["Distributed Computing", "https://csfrontiers.org/distributed-computing.html", Share2],
          ["IoT and Cybersecurity", "https://csfrontiers.org/iot-and-cybersecurity.html", Lock],
          ["AI and Machine Learning", "https://csfrontiers.org/ai-and-machine-learning.html", Brain],
          ["Software Engineering", "https://csfrontiers.org/software-engineering.html", Wrench],
        ].map(([name, url, Icon]) => <li className="mt-2 mb-2">
          <Icon className="mr-2" style={{display: "inline-block"}}/>
...
const CurriculumCard = ({ program }) => (
  <div className="group bg-white dark:bg-slate-800 transition-all duration-300 hover:shadow-xl flex flex-col rounded-2xl">
    <div className="p-6 flex flex-col flex-grow">
      ...
      <a href={program.href}>
        <button className={`text-white text-lg font-bold bg-gradient-to-r ${program.color} hover:scale-105 transition-transform duration-300 w-full rounded-2xl`}>
          {program.buttonText}
        </button>
      </a>
    </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-lg mb-12">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:divide-x divide-blue-200 dark:divide-slate-600">
              <FeatureColumn icon={Users} text="For various educational levels and skillsets" />
              <FeatureColumn icon={BookOpen} text="Curated tutorials, projects, and lesson ideas" />
              <FeatureColumn icon={Lightbulb} text="Engaging and interactive education experiences" />
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {curriculumPrograms.map((program, index) => <CurriculumCard key={index} program={program} />)}
          </div>
        </div>
      </main>
    </div>
